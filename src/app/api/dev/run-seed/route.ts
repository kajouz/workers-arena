/**
 * TEMPORARY endpoint — runs the production seed script against the production
 * database. Call once, then DELETE this file and redeploy.
 *
 * Usage: POST /api/dev/run-seed
 */
  ...
  try {
    ...
    for (const w of WORKERS) {
      ...
      if (existing) {
        ...
        log.push(`✓ Worker ${w.nameEn} exists`);
        continue;
      }
      ...
      if (!catId || !cityId || !areaId) {
        log.push(`⚠ Skipping ${w.nameEn}: missing FK`);
        continue;
      }
      ...
      const worker = await prisma.worker.create({
        data: {
          ...
          bioEn: `${w.nameEn} is a skilled ${w.categorySlug} professional.`,
          bioAr: `${w.nameAr} محترف ${w.categorySlug} ذو خبرة.`,
    ...
    for (let i = 0; i < BOOKINGS.length; i++) {
      ...
      if (existing) { log.push(`✓ Booking ${num} exists`); continue; }
      ...
      await prisma.workerLedgerEntry.create({
        data: {
          ...
          kind: "EARNING",
          status: "POSTED",
          amount: b.quote - b.fee,
          balanceAfter: b.quote - b.fee,
          currency: "USD",
          reason: `Earnings: ${b.title}`,
          createdAt: completedAt,
        },
      });
      log.push(`✓ Created booking ${num}`);
    }
    ...
    for (const [slug, workerId] of Object.entries(workerIds)) {
      ...
      if (!existing) {
        ...
        log.push(`✓ Granted 50 credits to ${slug}`);
      }
    }
    ...
    return NextResponse.json({
      ok: true,
      log,
      summary: { workers: workerCount, bookings: bookingCount, ledgerEntries: ledgerCount },
    });
  } catch (e) {
