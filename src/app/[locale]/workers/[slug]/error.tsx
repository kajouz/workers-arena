...
export default function WorkerError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("[workers/[slug] error]", error);
  }, [error]);
  return (
    <div className="flex min-h-[50dvh] flex-col items-center justify-center px-4 py-10 text-center">
      ...
      <h1 className="mt-4 text-2xl font-black text-ink-900 dark:text-ink-50">Worker profile unavailable</h1>
      <p className="mt-2 max-w-md text-sm text-ink-500 dark:text-ink-400">
        This profile could not be loaded. Please try again or browse other professionals.
      </p>
      {error.digest ? <p className="mt-2 font-mono text-xs text-ink-400" dir="ltr">{error.digest}</p> : null}
      <div className="mt-6 flex gap-3">
        <Button onClick={() => reset()}>Try again</Button>
        <Link href="/search"><Button variant="outline">Browse workers</Button></Link>
      </div>
    </div>
  );
}
