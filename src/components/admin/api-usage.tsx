  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full"><thead className="bg-ink-50 dark:bg-ink-950"><tr><th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Endpoint</th><th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Calls</th><th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Avg Response</th><th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">P95 Response</th><th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Error Rate</th></tr></thead>
          <tbody className="divide-y divide-ink-200 dark:divide-ink-800">{endpoints.map((ep) => (<tr key={ep.path} className="hover:bg-ink-50 dark:hover:bg-ink-950"><td className="px-4 py-4"><span className="px-2 py-0.5 text-xs font-medium rounded bg-blue-100 text-blue-800 me-2 dark:bg-blue-500/15 dark:text-blue-300">{ep.method}</span><span className="font-mono text-sm text-ink-900 dark:text-ink-50">{ep.path}</span></td><td className="px-4 py-4 text-sm font-medium text-ink-900 dark:text-ink-50">{formatNumber(ep.calls)}</td><td className="px-4 py-4"><span className={cn("text-sm font-medium", ep.avgResponse < 200 ? "text-green-600" : ep.avgResponse < 500 ? "text-yellow-600" : "text-red-600")}>{ep.avgResponse}ms</span></td><td className="px-4 py-4 text-sm text-ink-600 dark:text-ink-300">{ep.p95Response}ms</td><td className="px-4 py-4"><span className={cn("text-sm font-medium", ep.errorRate < 1 ? "text-green-600" : ep.errorRate < 5 ? "text-yellow-600" : "text-red-600")}>{ep.errorRate}%</span></td></tr>))}</tbody>
        </table></div>
      </div>
    </div>
  );
