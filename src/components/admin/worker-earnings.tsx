        ...
        {workers.map((worker) => (
          <div key={worker.id} className="border-b border-ink-200 dark:border-ink-800 last:border-b-0">
            <button onClick={() => setExpandedWorker(expandedWorker === worker.id ? null : worker.id)} className="w-full flex items-center justify-between p-4 hover:bg-ink-50 dark:hover:bg-ink-950">
              <div className="flex items-center gap-4"><div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white font-bold">{worker.workerName.charAt(0)}</div><div className="text-left"><p className="font-medium text-ink-900 dark:text-ink-50">{worker.workerName}</p><p className="text-sm text-ink-500 dark:text-ink-400">{worker.workerNameAr}</p></div></div>
              <div className="flex items-center gap-8 text-sm"><div className="text-right"><p className="font-bold text-green-600">${formatNumber(worker.totalEarnings)}</p><p className="text-ink-500 dark:text-ink-400">Earnings</p></div><div className="text-right"><p className="font-bold text-orange-600">${formatNumber(worker.pendingPayouts)}</p><p className="text-ink-500 dark:text-ink-400">Pending</p></div><div className="text-right"><p className="font-bold">{worker.completedBookings}</p><p className="text-ink-500 dark:text-ink-400">Bookings</p></div>{expandedWorker === worker.id ? <ChevronDown className="w-5 h-5 text-ink-400 dark:text-ink-500" /> : <ChevronRight className="w-5 h-5 text-ink-400 dark:text-ink-500" />}</div>
            </button>
