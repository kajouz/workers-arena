    ...
    <div className="space-y-6">
      ...
      {activeTab === "import" ? (
        <div className="bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 p-6">
          ...
          <div className="border-2 border-dashed border-ink-300 dark:border-ink-700 rounded-xl p-12 text-center hover:border-blue-400 transition-colors">
            <Upload className="w-12 h-12 text-ink-400 dark:text-ink-500 mx-auto mb-4" />
            <p className="text-ink-600 dark:text-ink-300 mb-2">Drag and drop a CSV file here, or click to browse</p>
            ...
            {importFile && (
              <div className="mt-4 flex items-center justify-center gap-2"><FileText className="w-4 h-4 text-green-600" /><span className="text-sm text-ink-900 dark:text-ink-50">{importFile.name}</span><button onClick={() => setImportFile(null)}><X className="w-4 h-4 text-ink-400 dark:text-ink-500" /></button></div>
            )}
          </div>
