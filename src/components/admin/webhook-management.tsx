    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full"><thead className="bg-gray-50"><tr><th className="text-left px-4 py-3 text-sm font-medium text-gray-600">URL</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-600">Events</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-600">Status</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-600">Success Rate</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-600">Last Triggered</th><th className="text-left px-4 py-3 text-sm font-medium text-gray-600">Actions</th></tr></thead>
          <tbody className="divide-y divide-gray-200">{webhooks.map((wh) => (<tr key={wh.id} className="hover:bg-gray-50"><td className="px-4 py-4"><p className="font-mono text-sm text-gray-900 truncate max-w-xs">{wh.url}</p></td><td className="px-4 py-4"><div className="flex flex-wrap gap-1">{wh.events.map((e) => <span key={e} className="px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded">{e}</span>)}</div></td><td className="px-4 py-4"><span className={cn("px-2 py-1 text-xs font-medium rounded-full", wh.status === "active" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800")}>{wh.status}</span></td><td className="px-4 py-4"><span className={cn("font-medium", wh.successRate > 95 ? "text-green-600" : "text-yellow-600")}>{wh.successRate}%</span></td><td className="px-4 py-4 text-sm text-gray-500">{wh.lastTriggered ? formatDateTime(wh.lastTriggered, locale) : "Never"}</td><td className="px-4 py-4"><div className="flex items-center gap-2"><button className="text-blue-600 hover:text-blue-700"><RefreshCw className="w-4 h-4" /></button><button className="text-red-600 hover:text-red-700"><Trash2 className="w-4 h-4" /></button></div></td></tr>))}</tbody>
        </table>
      </div>
    </div>
  );
