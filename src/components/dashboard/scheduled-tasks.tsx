...
const categoryColors: Record<string, string> = {
  maintenance: "bg-blue-100 text-blue-800 dark:bg-blue-500/15 dark:text-blue-300",
  notification: "bg-purple-100 text-purple-800 dark:bg-purple-500/15 dark:text-purple-300",
  analytics: "bg-green-100 text-green-800 dark:bg-green-500/15 dark:text-green-300",
  cleanup: "bg-orange-100 text-orange-800 dark:bg-orange-500/15 dark:text-orange-300",
  backup: "bg-ink-100 dark:bg-ink-800 text-ink-800 dark:text-ink-100",
};

const statusColors: Record<string, string> = {
  success: "bg-green-100 text-green-800 dark:bg-green-500/15 dark:text-green-300",
  failed: "bg-red-100 text-red-800 dark:bg-red-500/15 dark:text-red-300",
  running: "bg-yellow-100 text-yellow-800 dark:bg-yellow-500/15 dark:text-yellow-300",
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{stats.totalTasks}</p>
              <p className="text-sm text-ink-500 dark:text-ink-400">Total Tasks</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{stats.activeTasks}</p>
        ...
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{formatNumber(stats.totalRuns)}</p>
              <p className="text-sm text-ink-500 dark:text-ink-400">Total Runs</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{stats.successRate}%</p>
              <p className="text-sm text-ink-500 dark:text-ink-400">Success Rate</p>
            </div>
          </div>
        </div>
      </div>
            ...
            {filteredTasks.map((task) => {
              ...
              return (
                <div key={task.id} className={cn(
                  ...
                  task.isActive ? "border-ink-200 dark:border-ink-800 hover:shadow-md" : "border-ink-100 dark:border-ink-800 opacity-60"
                )}>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-semibold text-ink-900 dark:text-ink-50">{task.name}</h3>
                          <span className={cn("px-2 py-0.5 text-xs font-medium rounded-full", categoryColors[task.category])}>
                            {task.category}
                          </span>
                          {task.lastStatus === "failed" && (
                            <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-red-100 text-red-800 dark:bg-red-500/15 dark:text-red-300">
                              Last run failed
                            </span>
                          )}
                        </div>
                        <p className="text-sm text-ink-500 dark:text-ink-400 mt-1">{task.description}</p>
                        <p className="font-mono text-xs text-ink-400 dark:text-ink-500 mt-1">{task.command}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          task.isActive ? "bg-green-100 text-green-800 dark:bg-green-500/15 dark:text-green-300" : "bg-ink-100 dark:bg-ink-800 text-ink-800 dark:text-ink-100"
                        )}
                      >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                    <div>
                      <p className="text-ink-500 dark:text-ink-400">Schedule</p>
                      <p className="font-mono font-medium">{task.schedule}</p>
                    </div>
                    <div>
                      <p className="text-ink-500 dark:text-ink-400">Last Run</p>
                      <p className="font-medium">
                        {task.lastRun ? formatDateTime(task.lastRun, locale) : "Never"}
                      </p>
                    </div>
                    <div>
                      <p className="text-ink-500 dark:text-ink-400">Next Run</p>
                      <p className="font-medium">{formatDateTime(task.nextRun, locale)}</p>
                    </div>
                    <div>
                      <p className="text-ink-500 dark:text-ink-400">Avg Duration</p>
                      <p className="font-medium">
                        {task.averageDuration ? formatDuration(task.averageDuration) : "—"}
                      </p>
                    </div>
                    <div>
                      <p className="text-ink-500 dark:text-ink-400">Runs / Failures</p>
                      <p className="font-medium">
        ...
        <div className="bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full">
            <thead className="bg-ink-50 dark:bg-ink-950">
              <tr>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Task</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Started</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Duration</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Status</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Output</th>
              ...
              {history.map((item) => (
                <tr key={item.id} className="hover:bg-ink-50 dark:hover:bg-ink-950">
                  <td className="px-4 py-4">
                    <p className="font-medium text-ink-900 dark:text-ink-50">{item.taskName}</p>
                  </td>
                  ...
                  <td className="px-4 py-4 text-sm text-ink-600 dark:text-ink-300 max-w-xs truncate">
