...
const formatColors: Record<string, string> = {
  pdf: "bg-red-100 text-red-800 dark:bg-red-500/15 dark:text-red-300",
  csv: "bg-green-100 text-green-800 dark:bg-green-500/15 dark:text-green-300",
  excel: "bg-blue-100 text-blue-800 dark:bg-blue-500/15 dark:text-blue-300",
};
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{stats.totalSchedules}</p>
              <p className="text-sm text-ink-500 dark:text-ink-400">Total Schedules</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{stats.activeSchedules}</p>
              <p className="text-sm text-ink-500 dark:text-ink-400">Active</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-ink-900 dark:text-ink-50">{stats.reportsSent}</p>
              <p className="text-sm text-ink-500 dark:text-ink-400">Reports Sent</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-ink-900 rounded-xl p-4 border border-ink-200 dark:border-ink-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-ink-500 dark:text-ink-400">Last Sent</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            {filteredSchedules.map((schedule) => (
              <div key={schedule.id} className={cn(
                ...
                schedule.isActive ? "border-ink-200 dark:border-ink-800 hover:shadow-md" : "border-ink-100 dark:border-ink-800 opacity-60"
              )}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-ink-900 dark:text-ink-50">{schedule.name}</h3>
                      <span className={cn("px-2 py-0.5 text-xs font-medium rounded-full", formatColors[schedule.format])}>
                        {schedule.format.toUpperCase()}
                      </span>
                      <span className="px-2 py-0.5 text-xs font-medium rounded-full bg-ink-100 dark:bg-ink-800 text-ink-800 dark:text-ink-100">
                        {typeLabels[schedule.type]}
                      </span>
                    </div>
                    <p className="text-sm text-ink-500 dark:text-ink-400 mt-1">{schedule.description}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        schedule.isActive ? "bg-green-100 text-green-800 dark:bg-green-500/15 dark:text-green-300" : "bg-ink-100 dark:bg-ink-800 text-ink-800 dark:text-ink-100"
                      )}
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    <p className="text-ink-500 dark:text-ink-400">Last Sent</p>
                    <p className="font-medium">
                      {schedule.lastSent ? formatDate(schedule.lastSent, locale) : "Never"}
                    </p>
                  </div>
                  <div>
                    <p className="text-ink-500 dark:text-ink-400">Next Scheduled</p>
                    <p className="font-medium">{formatDate(schedule.nextScheduled, locale)}</p>
                  </div>
                  <div>
                    <p className="text-ink-500 dark:text-ink-400">Sections</p>
                ...
                <div className="mt-3 flex flex-wrap gap-1">
                  {schedule.sections.map((section) => (
                    <span key={section} className="px-2 py-0.5 text-xs bg-ink-100 dark:bg-ink-800 text-ink-600 dark:text-ink-300 rounded">
                      {section.replace(/_/g, " ")}
                    </span>
                  ))}
                </div>
        ...
        <div className="bg-white dark:bg-ink-900 rounded-xl border border-ink-200 dark:border-ink-800 overflow-hidden">
          <div className="overflow-x-auto"><table className="w-full">
            <thead className="bg-ink-50 dark:bg-ink-950">
              <tr>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Report</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Sent At</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Recipients</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Format</th>
                <th className="text-start px-4 py-3 text-sm font-medium text-ink-600 dark:text-ink-300">Size</th>
            ...
            <tbody className="divide-y divide-ink-200 dark:divide-ink-800">
              {history.map((item) => (
                <tr key={item.id} className="hover:bg-ink-50 dark:hover:bg-ink-950">
                  <td className="px-4 py-4">
                    <p className="font-medium text-ink-900 dark:text-ink-50">{item.scheduleName}</p>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      <button className="text-ink-400 dark:text-ink-500 hover:text-ink-600 dark:hover:text-ink-300" title="Download">
                        <Download className="w-4 h-4" />
                      </button>
                      <button className="text-ink-400 dark:text-ink-500 hover:text-ink-600 dark:hover:text-ink-300" title="View">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-dialog">
          <div className="bg-white dark:bg-ink-900 rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-ink-700 dark:text-ink-200 mb-1">Report Name</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-ink-700 dark:text-ink-200 mb-1">Frequency</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-ink-700 dark:text-ink-200 mb-1">Format</label>
                  <select
              ...
              <div>
                <label className="block text-sm font-medium text-ink-700 dark:text-ink-200 mb-2">Report Sections</label>
                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto border border-ink-200 dark:border-ink-800 rounded-lg p-3">
                  {reportSections.map((section) => (
                    <label key={section.id} className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-sm text-ink-700 dark:text-ink-200">{section.label}</span>
                    </label>
                  ))}
                </div>
              </div>
