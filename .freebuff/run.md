...
**Cron endpoints (deployed scheduler, same `CRON_SECRET` header):** `GET /api/cron/reminders` (daily — subscription expiry nudges + the booking "job starts tomorrow" reminders) and `GET /api/cron/recurring` (daily — recurring-generation: materializes the next batch of maintenance-contract occurrences, idempotent; demo mode no-ops). Both are covered by the nightly `db-smoke` job below — the smoke runs `runBookingReminderEngine` directly and the W2 recurring section calls `prismaGenerateRecurringOccurrences` (nothing due → 0, after availability → 1, re-run → 0), so the runner proves the engines that production cron hits.
