...
> **Living document.** This file is the single source of truth for what the product *is*, what it *could become*, and how we get there (web + mobile). Update it as features ship — every checklist item should reflect reality, not aspiration. See [How to keep this document fresh](#-how-to-keep-this-document-fresh).
...
- [ ] **Flip the repo seam to Prisma — W2: rest** — cities, suggestions, analytics, and all mutations (reviews, leads, subscriptions, campaigns, ads, notifications, activity, verification) still run on the demo dataset. Add trigram search index + Redis for hot queries.
...
- [ ] **Multi-candidate request-quotes** — a customer invites up to 3 workers to quote one job and picks a winner, reusing the `Booking` model (each invited worker is a slot-less `QUOTING`/`QUOTED` booking; the winner claims a slot through the existing CAS). Design: **[docs/multi-candidate-quotes.md](multi-candidate-quotes.md)**.
- [ ] **Customer ⇄ worker chat** — real-time messaging with quote sharing; WhatsApp deep-link fallback.
